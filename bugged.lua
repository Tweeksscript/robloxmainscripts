-- Optimized, Fixed & Explained Version
...
Title.Text = "Universal Utility Panel"
...
RunService.RenderStepped:Connect(function()
    ...
    for plr,v in pairs(ESP) do
        ...
        if Settings.ESP and hrp2 and h and h.Health>0 then
            ...
            if on then
                -- OLD ESP SCALE (smaller & distance-based)
                local scale = 1 / (pos.Z * math.tan(math.rad(Camera.FieldOfView * 0.5)) * 2) * 1000
                local w = math.clamp(4.5 * scale, 20, 300)
                local hg = math.clamp(6 * scale, 35, 400)
                box.Size = Vector2.new(w, hg)
                box.Position = Vector2.new(pos.X - w/2, pos.Y - hg/2)
                box.Color = Settings.TeamColors and (plr.Team==LocalPlayer.Team and Color3.fromRGB(0,255,0) or Color3.fromRGB(255,0,0)) or Color3.new(1,1,1)
                box.Visible = true
                name.Text = Settings.Distance and plr.Name.." ["..math.floor((hrp2.Position-hrp.Position).Magnitude).."]" or plr.Name
                name.Position = Vector2.new(pos.X, pos.Y - hg/2 - 14)
                name.Size = math.clamp(12 * scale, 12, 22)
                name.Visible = true
    ...
    if Settings.Aimlock and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
        ...
        if t then
            Camera.CFrame=Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position,t.Position),Settings.SmoothAim)
        end
    end
end)
